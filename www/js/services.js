...
.factory('Accounts', function ($http, $rootScope, $stateParams, $q, $window, _, config, blockapps, $ionicPlatform, ipfs_, lightwallet_, $localstorage) { //$cordovaKeychain, $cordovaTouchID
  ...
  var getPersona = function(address){
    ...
    //       console.log("hello blockapps too: " + s)
  ...
  return {
    ...
    signTx : function(p) {
      getAccount(getCurrentAddress()).then(function(account){
        ...
        blockapps.ethbase.Transaction(p.tx).send(account.privkey).then(function(r){
          //console.log("tx result: " + JSON.stringify(r))
        })
...
.factory('blockapps', ['$window', function($window) {
  ...
  //console.log('blockapps-js: ' + JSON.stringify(b))
  return b;
...
// }])
.constant('config', {
    strato :          "http://strato-dev3.blockapps.net",
    uri:              "http://strato-dev3.blockapps.net" + "/eth/v1.1",
    ...
    ipfsPort :        "5001",
    ipfsWebPort:      "8080",
    personaRegistry:  "7eb50c3309943d747ca3c3086b0f19dcdd5d64af"
  }
)
